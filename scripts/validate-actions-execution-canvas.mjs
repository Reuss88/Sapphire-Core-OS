...
const [workspace, shell, styles, journal] = await Promise.all([
  readFile("apps/web/components/actions/actions-workspace.tsx", "utf8"),
  readFile("apps/web/design-system/shell.tsx", "utf8"),
  readFile("apps/web/app/globals.css", "utf8"),
  readFile("apps/web/components/actions/work-journal.tsx", "utf8"),
]);

for (const required of ["execution-brief", "lens-strip", "queue-pane", "context-canvas", "journal-pane", "TabCollection", "SapphireShell"]) assert.match(workspace, new RegExp(required));
for (const required of ["brand-rail", "command-header", "SAPPHIRE_NAV_PIN_STORAGE_KEY", "Open workspace navigation"]) assert.match(shell, new RegExp(required));
...
assert.match(shell, /sessionStorage\.setItem\(SAPPHIRE_NAV_PIN_STORAGE_KEY/);
