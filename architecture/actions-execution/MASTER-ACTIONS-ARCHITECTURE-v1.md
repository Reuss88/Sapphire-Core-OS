...
Inbox owns communication threads/messages. Actions may create follow-ups from Inbox and link to threads.
...
Changes should invalidate affected queues in realtime. Automation must be idempotent. Scheduled escalation may create notifications/events but must not manufacture approvals or authority.

## Non-negotiables
- no frontend-only completion state;
- no duplicate approval system;
- no silent ownership changes;
- no completion without evidence where policy requires evidence;
- no AI assignment/approval without a traceable authorised rule or user action;
- no hard deletion of audited execution history.
